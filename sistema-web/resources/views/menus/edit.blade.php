    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="mb-0">Editar Menú: {{ $menu->nombre }}</h4>
            </div>
            <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('nombre') is-invalid @enderror"
                                id="nombre" name="nombre" value="{{ old('nombre', $menu->nombre) }}" required>
                            @error('nombre')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('icono') is-invalid @enderror"
                                id="icono" name="icono" value="{{ old('icono', $menu->icono) }}"
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" class="form-control @error('orden') is-invalid @enderror"
                                id="orden" name="orden" value="{{ old('orden', $menu->orden) }}" min="0" required>
                            @error('orden')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('id_padre') is-invalid @enderror" id="id_padre" name="id_padre">
                                ...
                                @foreach ($menusPadre as $padre)
                                    <option value="{{ $padre->id_menu }}"
                                        {{ old('id_padre', $menu->id_padre) == $padre->id_menu ? 'selected' : '' }}>
                                        {{ $padre->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select @error('mostrar_en') is-invalid @enderror" id="mostrar_en" name="mostrar_en" required>
                                <option value="sidebar" {{ old('mostrar_en', $menu->mostrar_en) == 'sidebar' ? 'selected' : '' }}>Sidebar</option>
                                <option value="header" {{ old('mostrar_en', $menu->mostrar_en) == 'header' ? 'selected' : '' }}>Header</option>
                                <option value="ambos" {{ old('mostrar_en', $menu->mostrar_en) == 'ambos' ? 'selected' : '' }}>Ambos</option>
                            </select>
                            @error('mostrar_en')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            ...
                            <select class="form-select @error('id_rol') is-invalid @enderror" id="id_rol" name="id_rol">
                                ...
                                @foreach ($roles as $rol)
                                    <option value="{{ $rol->id_rol }}"
                                        {{ old('id_rol', $menu->id_rol) == $rol->id_rol ? 'selected' : '' }}>
                                        {{ $rol->nombre_rol }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-12 mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="activo" name="activo"
                                    value="1" {{ old('activo', $menu->activo) ? 'checked' : '' }}>
